...
class CleanDataPipeline:
    ...
    def __init__(self, focus_areas_config_path: Optional[Path] = None):
        ...
        if not focus_areas_config_path:
            ...
            # Check if we're in vendored code (skill)
            if 'vendored' in str(current_file):
                # We're in .claude/skills/chrome-update-digest/vendored/processors/
                # Go up to skill root: vendored/ -> chrome-update-digest/
                skill_root = current_file.parent.parent.parent
                focus_areas_config_path = skill_root / 'config' / 'focus_areas.yaml'
            else:
                # We're in src/chrome_update_digest/processors/
